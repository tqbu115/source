	...
	if(isset($_GET['id'])){
		...
		if($d->num_rows()>0){
			...
			while($row = $d->fetch_array()){
				...
				delete_file(_upload_album.$row['photo']);
				$sql = "select photo from #_album_photo where id_album='".$row['id']."' and type='".$_GET['type']."' order by stt,id desc ";
				$d->query($sql);
				$ds_photo = $d->result_array();	
				foreach ($ds_photo as $key => $photo) {
					delete_file(_upload_album.$photo['photo']);
				}
				//delete_file(_upload_album.$row['thumb']);
