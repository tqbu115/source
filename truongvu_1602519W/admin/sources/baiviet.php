...
function get_man(){
	global $d, $item,$ds_tags,$ds_photo,$ds_photo2;
	...
	$d->reset();
	$sql = "select * from #_baiviet_photo where id_baiviet='".$id."' and type='duan_keo' order by stt,id desc ";
	$d->query($sql);
	$ds_photo2 = $d->result_array();	
}
function save_man(){
	global $d,$arr_lang;
	$file_name=images_name($_FILES['file']['name']);
	$file_name360=images_name($_FILES['file360']['name']);
	$file_name_icon=images_name($_FILES['file']['icon']);
	...
	if($id){
		...
		if($photo360 = upload_image("file360", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_baiviet,$file_name360)){
			...
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_baiviet.$row['photo360']);		
			}
		}
		if($icon = upload_image("icon", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_baiviet,$file_name_icon)){
			$data['icon'] = $icon;		
			$d->setTable('baiviet');
			$d->setWhere('id', $id);
			$d->select();
		...
		if($d->update($data)){
			if (isset($_FILES['files'])) {
				for($i=0;$i<count($_FILES['files']['name']);$i++){
					if($_FILES['files']['name'][$i]!=''){
						...
						$data1['ten'] = $_POST['tens'][$i];
						$data1['mota'] = $_POST['motas'][$i];
			...
			if (isset($_FILES['files2'])) {
				for($i=0;$i<count($_FILES['files2']['name']);$i++){
					if($_FILES['files2']['name'][$i]!=''){
						$file['name'] = $_FILES['files2']['name'][$i];
						$file['type'] = $_FILES['files2']['type'][$i];
						$file['tmp_name'] = $_FILES['files2']['tmp_name'][$i];
						$file['error'] = $_FILES['files2']['error'][$i];
						$file['size'] = $_FILES['files2']['size'][$i];
						$file_name = images_name($_FILES['files2']['name'][$i]);
						$photo = upload_photos($file, 'jpg|png|gif|PNG|GIF|JPG|JPEG|jpeg', _upload_baiviet,$file_name);
						$data1['photo'] = $photo;
						$data1['thumb'] = create_thumb($data1['photo'], _width_thumb, _height_thumb, _upload_baiviet,$file_name,_style_thumb);	
						$data1['stt'] = (int)$_POST['stthinh'][$i];
	...
	if(isset($_GET['id'])){
		...
		if(count($photo_lq)>0){
			for($i=0;$i<count($photo_lq);$i++){
				...
				delete_file(_upload_baiviet.$photo_lq[$i]['icon']);
				delete_file(_upload_baiviet.$photo_lq[$i]['photo360']);
			}
		}
		...
		for ($i=0 ; $i<count($listid) ; $i++){
			...
			if(count($photo_lq)>0){
				for($j=0;$j<count($photo_lq);$j++){
					...
					delete_file(_upload_baiviet.$photo_lq[$j]['icon']);
					delete_file(_upload_baiviet.$photo_lq[$j]['photo360']);
				}
			}
			$sql = "delete from #_baiviet_photo where id_baiviet='".$id."'";
			$d->query($sql);
