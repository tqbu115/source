...
function save_item(){
	...
	if($id){
		...
		if($_POST['id_tinhtrang'] == 5){
			$sql = "select * from #_order where id='".$_POST['id']."'";
			$d->query($sql);
			if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=order&act=man");
			$item = $d->fetch_array();
			
			$d->reset();
			$sql="select id_product,soluong from #_order_detail where id_order = '".$item['id']."'";
			$d->query($sql);
			$result_ctdonhang=$d->result_array();

			foreach ($result_ctdonhang as $key => $value) {
				soluongsp($value['id_product'],$value['soluong']);
			}
		}
