...
function save_city(){
	...
	if($id){
		$id =  themdau($_POST['id']);					
		$data['ten'] = $_POST['name'];	
		$data['thoigiangiaohang'] = $_POST['thoigiangiaohang'];	
...
function get_dists(){
	...
	$sql = "SELECT * from #_place_dist $where order by stt asc  limit $bg,$pageSize";		
	$d->query($sql);
	$items = $d->result_array();	
	$url_link='index.php?com=place&act=man_dist'.$more;		
...
function get_wards(){
	...
	$d->query($sql);
	$items = $d->result_array();	
	$url_link='index.php?com=place&act=man_ward'.$more;		
...
function save_ward(){
	...
	if($id){
		...
		{		
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=place&act=man_ward&curPage=".$_REQUEST['curPage']."");
	}
}
