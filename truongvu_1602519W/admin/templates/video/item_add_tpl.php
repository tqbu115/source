	...
	<div class="widget">
		...
		<div class="formRow">
			...
			<div class="formRight">
				...
				<div class="mt10"><img src="<?=_upload_video.$item['photo']?>"  alt="NO PHOTO" width="100" /></div>
			</div>
			<div class="clear"></div>
		</div>
        ...
        <?php foreach ($arr_lang as $key => $l) { ?>
			...
			<div class="formRow lang_hidden lang_<?=$l?> <?=$key==0?'active':''?>">
				...
				<div class="formRight">
					<textarea rows="4" name="mota_<?=$l?>" title="Mô Tả <?=$config['langs'][$l]?>" id="mota_<?=$l?>" class="tipS validate[required]"><?=@$item['mota_'.$l]?></textarea>
				</div>
				<div class="clear"></div>
			</div>
			<?php } ?>
