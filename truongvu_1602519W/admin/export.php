...
// Add some data
    ...
    for($i=0,$count=count($items);$i<$count;$i++) { 
    	...
    	$objPHPExcel->setActiveSheetIndex( 0 )
    	->setCellValue( 'A'.$vitri, $i+1 )->setCellValue( 'B'.$vitri,$items[$i]['ten'] )
    	->setCellValue( 'C'.$vitri, $items[$i]['soluong'])
    	->setCellValue( 'D'.$vitri, number_format($items[$i]['gia']) )
    	->setCellValue( 'E'.$vitri, number_format($items[$i]['gia']*$items[$i]['soluong']) );
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue( 'E'.$vitri,number_format($result_giohang['tonggia']));
