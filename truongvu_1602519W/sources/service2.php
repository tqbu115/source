<?php  if(!defined('_source')) die("Error");
		...
		if($id!=''){
			...
			$sql = "select ten_$lang,mota_$lang,noidung_$lang,ngaytao,photo,id,photo360,thongtin_$lang from #_baiviet where hienthi=1 and tenkhongdau='".$id."'";
			$d->query($sql);
			...
			$d->reset();
			$sql = "select * from #_baiviet_photo where hienthi=1 and id_baiviet='".$row_detail['id']."' and type='duan_keo' order by stt,id desc limit 0,2";
			$d->query($sql);
			$hinhanh_keo = $d->result_array();
			...
			$sql = "select ten_$lang,thumb,tenkhongdau,id,ngaytao,mota_$lang,photo,link from $where $limit";
			$d->query($sql);
			$tintuc = $d->result_array();
