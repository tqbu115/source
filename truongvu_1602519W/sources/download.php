<?php  if(!defined('_source')) die("Error");
		...
		if($id!=''){
			...
			$sql = "select id from #_download_list where hienthi=1 and type='".$type_bar."' and tenkhongdau='".$idl."'";
			$d->query($sql);
			$download_list = $d->fetch_array();

			$sql = "select * from #_download where hienthi=1 and type='".$type_bar."' and id_list='".$download_list['id']."'";
			$d->query($sql);
			$tintuc = $d->result_array();
