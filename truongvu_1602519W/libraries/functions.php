...
function show_tags($tags,$lass = "tags"){
	...
	for ($i=0,$count=count($arr); $i < $count; $i++) { 
		...
		echo '<a href="'.$row["tenkhongdau"].'" class="'.$lass.'">'.$row["ten_vi"].'</a>';
...
function getname($tbl,$id,$type){

	global $d,$lang;

	$sql = "select ten_$lang from table_$tbl where id='".$id."' ";
...
function soluongsp($id,$sl){
	global $d;
	$sql="update #_product set soluong = ( soluong + ".$sl." ) where id='$id' ";
	$d->query($sql);
...
function make_date($time,$lang='vi'){
...

{
...

    {
        ...
        if ($first_btn && $this->cur_page > 1) {
            ...
            $msg .= "<li p='1' class='active'><i class='fa fa-angle-double-left' aria-hidden='true'></i></li>";

        } else if ($first_btn) {

            $msg .= "<li p='1' class='$this->class_inactive'><i class='fa fa-angle-double-left' aria-hidden='true'></i></li>";
        ...
        if ($previous_btn && $this->cur_page > 1) {
            ...
            $msg .= "<li p='$pre' class='active'><i class='fa fa-angle-left' aria-hidden='true'></i></li>";

        } else if ($previous_btn) {

            $msg .= "<li class='$this->class_inactive'><i class='fa fa-angle-left' aria-hidden='true'></i></li>";
        ...
        if ($next_btn && $this->cur_page < $no_of_paginations) {
            ...
            $msg .= "<li p='$nex' class='active'><i class='fa fa-angle-right' aria-hidden='true'></i></li>";

        } else if ($next_btn) {

            $msg .= "<li class='$this->class_inactive'><i class='fa fa-angle-right' aria-hidden='true'></i></li>";
        ...
        if ($last_btn && $this->cur_page < $no_of_paginations) {
            ...
            $msg .= "<li p='$no_of_paginations' class='$this->class_active'><i class='fa fa-angle-double-right' aria-hidden='true'></i></li>";

        } else if ($last_btn) {

            $msg .= "<li p='$no_of_paginations' class='$this->class_inactive'> <i class='fa fa-angle-double-right' aria-hidden='true'></i> </li>";
