...
function get_video_youtube($width=2,$height=1,$select=true,$auto=false ){
	...
	$chuoi= getYoutubeIdFromUrl($video[0]['links']);

	//showtext div load

	$showtext .= '<div style="padding-bottom: '.$ratio.'%;position:relative;width:100%;height: 0;" id="box_video" data-id="'.$chuoi.'" data-width="'.$width.'" data-height="'.$height.'" data-auto="'.$auto.'" ></div>';

	if($select){

		$showtext .= '<div class="select_video"><select width="100%" id="video_lienquan">';

		$showtext .='<option value="">-- List video --</option>';

		for ($i=0,$count=count($video); $i < $count; $i++) { 

			$chuoi= getYoutubeIdFromUrl($video[$i]['links']);

			$showtext .='<option value="'.$chuoi.'">'.$video[$i]['ten'].'</option>';
...
function get_video_youtube_2($width=2,$height=1,$select=true,$auto=false ){
	...
	if($select){
		...
		for ($i=0,$count=count($video); $i < $count; $i++) { 
			...
			$showtext .='<div class="video_thumb" data-id="'.$chuoi.'">';

			$showtext .='<img src="http://img.youtube.com/vi/'.$chuoi.'/default.jpg" border="0" align="left" />';
